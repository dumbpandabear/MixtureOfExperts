...
class MoE(nn.Module):
    ...
    def moe_infer(self,
        ...
        """
        Forward pass through Mixture of Experts routing.

        NOTE: ADDS SLIGHT OPTIMIZATIONS OVER THE REFERENCE CODE; but doesn't improve the running time.

        Args:
            x (Tensor): [num_tokens, hidden_dim] input tokens.
            flat_expert_indices (Tensor): [num_tokens * n_experts_per_token] expert indices for each token.
            flat_expert_weights (Tensor): [num_tokens * n_experts_per_token] gating weights for each token.

        Returns:
            Tensor: [num_tokens, hidden_dim] aggregated expert output.
        """
        device = x.device
        ...
        # Sort expert indices to make batch processing per expert easier
        idxs = flat_expert_indices.argsort()
        sorted_expert_ids = flat_expert_indices[idxs]

        # Count how many tokens go to each expert
        counts = torch.bincount(sorted_expert_ids, minlength=n_experts).cpu().numpy()
        tokens_per_expert = counts.cumsum()

        # Map each position in flat_expert_indices to a token in x
        token_idxs = (idxs // num_per_tok).to(device)
        ...
        for expert_id, end_idx in enumerate(tokens_per_expert):
            ...
            # Get expert and relevant tokens
            expert = self.experts[expert_id]
            exp_token_idxs = token_idxs[start_idx:end_idx]
            expert_inputs = x[exp_token_idxs]
            ...
            if weights.dim() == 1:
                weights = weights.unsqueeze(-1)  # Make sure it's [N, 1]
            expert_outputs.mul_(weights)


            # Accumulate outputs into the expert_cache using scatter reduce
            expert_cache.scatter_reduce_(
                dim=0,
                index=exp_token_idxs.view(-1, 1).expand(-1, hidden_dim),
                src=expert_outputs,
                reduce='sum'
            )
...
# check_implementation = make_match_reference(custom_kernel, rtol=1e-2, atol=1e-2)